...
func TestAuthorizationService_WriteAuthorization(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			m.UpdateAuthorizatioFn = func(ctx context.Context, id influxdb.ID, upd *influxdb.AuthorizationUpdate) error {
				return nil
			}
			s := authorizer.NewAuthorizationService(m)
			...
			t.Run("update authorization", func(t *testing.T) {
				err := s.UpdateAuthorization(ctx, 10, &influxdb.AuthorizationUpdate{Status: influxdb.Inactive.Ptr()})
				influxdbtesting.ErrorsEqual(t, err, tt.wants.err)
			})
